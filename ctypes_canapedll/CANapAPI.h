...
#define AEC_OBJECT_TYPE_DOESNT_MATCH  13 //!< Object type doesn't match
...
#define AEC_EXIT_DENIED_WHILE_ACQU    21 //!< Asap3Exit denied because data acquisition is still running
...
#define AEC_FIFO_ALREADY_INIT         54  //!< The FIFO Memory was already created. Close all connections to reconfigure.
...
#define AEC_CANAPE_INVALID_PRG_PATH   63  //!< CreateProcess of CANape failed: program directory not accessible/nonexistent
...
#define ACE_ERR_UNSUPPORTED_TYPE     136  //!< Unsupported data type for measurement
...
//! Type definition for TScriptStatus (has to be compatible to VFunctionStatus in fun.h)
enum TScriptStatus
{
  ...
  eTScrDelayedCompiling, //!< Waiting in the list for compilation - Task will be recompiled after mWakeUpTime 
                         //!< (support of asynchronous loading of MDF-files/Databases)
  eTScrException       //!< Exception during asynchronous calling of DLL function
};

//! possible Modes for page switching
enum e_RamMode
{
...
//! Type definition for Recorder types
enum TRecorderType
{
...
//! Type definition for Callback Event IDs
...
struct  DBFileInfo  //! DBFileInfo structure
{
  ...
  unsigned char type;                /*!< type of the Database file: \n  
                            ...
                            Update = 15, CDP = 16, LostVariable = 17, PDX = 18,
...
/*! \param hdl \input Asap3 Handle
 *  \param recorderID \input Pointer to the Recorder ID, see Asap3DefineRecorder()
 *  \param module \input Pointer to a defined module
...
/*! \param hdl \input Asap3 Handle
 *  \param recorderID \input Pointer to the Recorder ID, see Asap3DefineRecorder()
 *  \param NetworkName \input Pointer Network which should be logged
 *  \param enable \ enables or disables the logging
 */
...
/*! \param hdl \input Asap3 Handle
 *  \param recorderID \input Pointer to the Recorder ID, see Asap3DefineRecorder()
 *  \param NetworkName \input Pointer Network which should be checked
...
/*! \param hdl \input Asap3 Handle
 *  \param recorderID \input Pointer to the Recorder ID, see Asap3DefineRecorder()
 *  \param module \input Pointer to the device which should be checked
 *  \param enable \ return true if logging is enabled
 */
 ...
 *  @ingroup ApiFunctions
 *  provides access to CANape scripts
 *  @{
 */
...
//! Returns the exit code of a script
// ~~~~~~~~~~~~~~~~~~
/*! The type of the exit code is always double
 * \param hdl \input Asap3 Handle
 * \param scriptFile \input Handle of the script returned by Asap3ExecuteScriptEx
 * \param Value \output Exit code value of the script
 */
 ...
 *  @ingroup ApiFunctions
 *  provides diagnostic access to devices
 *  @{
 */
 ...
 *  \param PrivateData \input User defined PrivateData
 *
*/
 ...
 *  @ingroup ApiFunctions
 *  provides flash access to devices
 *  @{
 */
 ...
 *  \param Name \output buffer size of the parameter name
 ...
 *  \param JobName \input name of the job or a flash script
...
//!  Asks for a Flash job current state
/*! \param hdl \input Asap3 handle
 *  \param module \input points to demanded module.
 *  \param isRunning  \output isRunning  TRUE = yes FALSE = no
 *  \param Progress   \output progress of the flash job in percent
...
//!  Callback function ONMEASUERMENT_START
/*! \param hdl \input Asap3 handle
